...
perform_dqtest <- function(x,
  ...
  if (!is.character(var_name)) {
    stop("If supplied the var_name argument must be character.")
  }
  ...
  if (classification == "cont") { ############################################################################################
    ...
    if (!is.null(exclude_values)) {
      if (!is.numeric(exclude_values)) {
        stop("For continuous input the values to be excluded must be numeric.")
      }
      x <- x[!(x %in% exclude_values)]
    }
    if (!is.null(exclude_smaller_than)) {
      if (!is.numeric(exclude_smaller_than)) {
        stop("For continuous input the bounds for values to be excluded must be numeric.")
      }
      x <- x[!(x < exclude_smaller_than)]
    }
    if (!is.null(exclude_greater_than)) {
      if (!is.numeric(exclude_greater_than)) {
        stop("For continuous input the bounds for values to be excluded must be numeric.")
      }
    ...
    if (length(x) == length_original) {
      ...
      output_list$exclusions <- list(
        rel_excluded = 1 - (length(x) / length_original),
    ...
    if (is.null(range_min) && is.null(range_max)) {
      ...
      if (!(is.null(range_min) | is.numeric(range_min))) {
        stop("The ranges for continuous input must be numeric if given.")
      }
    ...
    # diagnostic plots:
    output_list$boxplot <- cont_boxplot(
      x = x,
      var_name = var_name,
      color = plot_col1
    )
    output_list$hist <- hist_cont(
      x = x,
      var_name = var_name,
      color = plot_col1,
    ...
    # exclude values if requested
    if (!is.null(exclude_values)) {
      if (!is.character(exclude_values)) {
        stop("exclude_values must be a character vector.")
      }
    ...
    if (length(x_factor) == length_original) {
      ...
      output_list$exclusions <- list(
        rel_excluded = 1 - (length(x_factor) / length_original),
    ...
    if (is.null(categories)) {
      ...
      if (!is.character(categories)) {
        stop("categories must be supplied as a character vector.")
      }
      output_list$cat_check <- dplyr::if_else(
        all(levels(x_factor) %in% categories),
    ...
    # exclude values if requested
    if (!is.null(exclude_values)) {
      if (!lubridate::is.Date(exclude_values)) {
        stop("For Date input the values to be excluded must be given as Date.")
      }
      x <- x[!(x %in% exclude_values)]
    }
    if (!is.null(exclude_smaller_than)) {
      if (!lubridate::is.Date(exclude_smaller_than)) {
        stop("For Date input the bounds for values to be excluded must be Dates")
      }
      x <- x[!(x < exclude_smaller_than)]
    }
    if (!is.null(exclude_greater_than)) {
      if (!lubridate::is.Date(exclude_greater_than)) {
        stop("For Date input the bounds for values to be excluded must be Dates")
      }
      x <- x[!(x > exclude_greater_than)]
    }
    ...
    if (is.null(range_min) && is.null(range_max)) {
      ...
      if (!(is.null(range_min) | lubridate::is.Date(range_min))) {
        stop("The ranges for Date input must be Dates if given.")
      }
    ...
    # diagnostic plots
    output_list$hist_date <- hist_date(
      x = x,
      var_name = var_name,
      color = plot_col1,
      range_col = ifelse(is.null(plot_col2), "black", plot_col2),
    ...
    output_list$hist_year <- hist_cont(
      x = lubridate::year(x),
      var_name = var_name,
      color = plot_col1,
      range_col = ifelse(is.null(plot_col2), "black", plot_col2),
    ...
    output_list$hist_month <- hist_cont(
      x = lubridate::month(x),
      var_name = var_name,
      color = plot_col1,
      range_col = ifelse(is.null(plot_col2), "black", plot_col2),
      ...
      range_max = NULL,
      rel = rel
    ) + coord_cartesian(xlim = c(1, 12)) + scale_x_continuous(breaks = 1:12)
    output_list$hist_week <- hist_cont(
      x = lubridate::week(x),
      var_name = var_name,
      color = plot_col1,
      range_col = ifelse(is.null(plot_col2), "black", plot_col2),
      ...
      range_max = NULL,
      rel = rel
    ) + coord_cartesian(xlim = c(0, 53)) + scale_x_continuous(breaks = (1:30) * 5)
    output_list$hist_wday <- bar_cat(
      x_factor = lubridate::wday(x = x, label = TRUE, abbr = FALSE),
      var_name = var_name,
      categories = NULL,
    ...
    output_list$hist_day <- hist_cont(
      x = lubridate::day(x),
      var_name = var_name,
      color = plot_col1,
      range_col = ifelse(is.null(plot_col2), "black", plot_col2),
      ...
      range_max = NULL,
      rel = rel
    ) + coord_cartesian(xlim = c(1, 31)) + scale_x_continuous(breaks = (1:20) * 2)
  } else if (classification == "datetime") { #################################################################################
    # convert into POSIXct (from potentially POSIXlt) and adjust timezone
    x <- as.POSIXct(x)
    # exclude values if requested
    if (!is.null(exclude_values)) {
      if (!lubridate::is.POSIXct(exclude_values)) {
        stop("For datetime input the values to be excluded must be given as POSIXct.")
      }
      x <- x[!(x %in% exclude_values)]
    }
    if (!is.null(exclude_smaller_than)) {
      if (!lubridate::is.POSIXct(exclude_smaller_than)) {
        stop("For datetime input the bounds for values to be excluded must be POSIXct.")
      }
      x <- x[!(x < exclude_smaller_than)]
    }
    if (!is.null(exclude_greater_than)) {
      if (!lubridate::is.POSIXct(exclude_greater_than)) {
        stop("For datetime input the bounds for values to be excluded must be POSIXct.")
      }
      x <- x[!(x > exclude_greater_than)]
    }
    ...
    if (is.null(range_min) && is.null(range_max)) {
      ...
      if (!(is.null(range_min) | lubridate::is.POSIXct(range_min))) {
        stop("The ranges for datetime input must be POSIXct if given.")
      }
    ...
    # diagnostic plots
    output_list$hist_datetime <- hist_datetime(
      x = x,
      var_name = var_name,
      color = plot_col1,
      range_col = ifelse(is.null(plot_col2), "black", plot_col2),
    ...
    output_list$hist_hour <- hist_cont(
      x = lubridate::hour(x),
      var_name = var_name,
      color = plot_col1,
      range_col = ifelse(is.null(plot_col2), "black", plot_col2),
      ...
      range_max = NULL,
      rel = rel
    ) + coord_cartesian(xlim = c(0, 24)) + scale_x_continuous(breaks = (0:12) * 2)
    output_list$hist_minute <- hist_cont(
      x = lubridate::minute(x),
      var_name = var_name,
      color = plot_col1,
      range_col = ifelse(is.null(plot_col2), "black", plot_col2),
      bin_num = bin_num,
      ...
      range_max = NULL,
      rel = rel
    ) + coord_cartesian(xlim = c(0, 60)) + scale_x_continuous(breaks = (1:20) * 5)
    output_list$hist_second <- hist_cont(
      x = lubridate::second(x),
      var_name = var_name,
      color = plot_col1,
      range_col = ifelse(is.null(plot_col2), "black", plot_col2),
      bin_num = bin_num,
